...
/*   Updated: 2025/05/09 21:01:54 by bmynbyae         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 */
int	handle_double_char_operator(t_parse_state *s, t_token **list)
{
	char	current_char;
	char	next_char;
	char	op_buffer[3];
...
int	handle_operator_or_space(t_parse_state *s, t_token **list)
{
	if (s->line[s->i + 1] != '\0')
	{
		if (handle_double_char_operator(s, list))
			return (1);
	}
	if (handle_single_char_operator_or_space(s, list))
		return (1);
	return (0);
}
 ...
 */
void	process_line_character(t_parse_state *state, t_token **token_list)
{
	int	is_char_quoted;
	...
	if (!is_char_quoted)
	{
		if (handle_operator_or_space(state, token_list))
			return ;
	}
